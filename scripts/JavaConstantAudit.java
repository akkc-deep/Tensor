...
/** Run with: java scripts/JavaConstantAudit.java [data-plane]. Requires JDK 21. */
public final class JavaConstantAudit {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (var entry : values.entrySet()) {
            if (entry.getValue().size() < 2 || entry.getValue().stream().allMatch(Occurrence::constant)) continue;
            duplicates++;
        ...
        System.out.println("Scanned " + files.size() + " production Java files; duplicate values: " + duplicates);
        System.out.println("Excluded literals: " + exclusions);
        if (duplicates != 0) System.exit(1);
    }
        ...
        for (TreePath current = path.getParentPath(); current != null; current = current.getParentPath()) {
            ...
            if (tree instanceof MethodTree method) {
                ...
                if (name.equals("message")
                        || file.equals("TushareBatchPolicies.java") && name.equals("failure")

...
it('shows the URL task, all historical facts and dynamic counts with full bigint values', async () => {
  ...
  expect(wrapper.get('h2').text()).toBe('daily')
  ...
  expect(wrapper.text()).toContain('已结束包含失败批次')
...
it.each(['invalid', 'not-found', 'failure'])('shows %s directly with safe recovery', async (state) => {
  ...
  expect(wrapper.findComponent(DownloadBatchTable).exists()).toBe(state === 'failure')
  expect(wrapper.get('a').attributes('href')).toBe('/downloads')
...
it('keeps successful batch data accessible when the initial detail request fails and can recover', async () => {
  api.getDownloadTask.mockRejectedValueOnce(new ClientError('NETWORK', REQUEST_ID))
  api.listDownloadTaskBatches.mockResolvedValue({ page: 1, pageSize: 20, total: 7n, items: [] })
  await render()
  expect(wrapper.text()).toContain('任务加载失败')
  expect(wrapper.getComponent(DownloadBatchTable).props('result').total).toBe(7n)
  await wrapper.get('[data-refresh-task]').trigger('click')
  await flushPromises()
  expect(wrapper.get('[data-task-status]').text()).toBe('部分失败')
})

it('calculates ended progress using exact large counts and leaves an unready plan unmeasured', async () => {
  const current = task('runningTask')
  api.getDownloadTask.mockResolvedValueOnce(task('runningTask', { counts: { ...current.counts,
    totalBatches: 9007199254740993n, succeededBatches: 4503599627370496n,
    failedBatches: 0n, pendingBatches: 4503599627370496n, runningBatches: 1n } }))
  await render()
  expect(wrapper.get('progress').attributes('value')).toBe('49.99')
  expect(wrapper.text()).toContain('已结束 4503599627370496 / 当前计划 9007199254740993 批')
  api.getDownloadTask.mockResolvedValueOnce(task('unplannedFailedTask'))
  await wrapper.get('[data-refresh-task]').trigger('click')
  await flushPromises()

...
it.each(['success', 'failure'])('ignores stale capability %s after a newer API selection', async outcome => {
  const old = deferred()
  ...
  if (outcome === 'success') old.resolve({ ...capabilities(), single: { available: false, parameters: [] } })
  else old.reject(new ClientError('NETWORK', 'stale-request'))
  expect(await stale).toBe(false)
  expect(flow.selectedApiName.value).toBe('weekly')
  expect(flow.capabilities.value.single.available).toBe(true)
  expect(flow.metadataState.value).toBe('READY')
...
it('blocks API selection for an unavailable source even when called directly', async () => {
  api.listDataSources.mockResolvedValueOnce([{ ...source(), downloadAvailable: false, unavailableReason: '凭证未配置' }])
  api.listApis.mockResolvedValueOnce([descriptor()])
  const flow = useDownloadFlow()
  await flow.load()
  await flow.selectSource('contract_fixture')
  ...
  expect(flow.capabilities.value).toBeNull()
  expect(flow.canSubmit.value).toBe(false)
  expect(api.getDownloadCapabilities).not.toHaveBeenCalled()
})

it.each(['success', 'failure'])('ignores stale source catalog %s after switching away', async outcome => {
  const old = deferred()
  api.listDataSources.mockResolvedValueOnce([source(), { ...source(), pluginId: 'other' }])
  api.listApis.mockReturnValueOnce(old.promise).mockResolvedValueOnce([{ ...descriptor(), apiName: 'other_api' }])
  const flow = useDownloadFlow()
  await flow.load()
  const stale = flow.selectSource('contract_fixture')
  await flow.selectSource('other')
  if (outcome === 'success') old.resolve([descriptor()])
  else old.reject(new ClientError('NETWORK', 'old-catalog'))
  expect(await stale).toBe(false)
  expect(flow.selectedPluginId.value).toBe('other')
  expect(flow.apis.value.map(api => api.apiName)).toEqual(['other_api'])
  expect(flow.metadataError.value).toBeNull()
  expect(flow.metadataState.value).toBe('READY')
})

...
it('preserves batch filters across pages, polling and retry, then resets them on a new task', async () => {
  api.getDownloadTask.mockResolvedValue(task('partialFailedTask'))
  flow = useDownloadTask()
  await flow.load(ID)
  await flow.changePage(3)
  await flow.changeBatchFilters({ status: 'FAILED', includeSplit: true })
  expect(api.listDownloadTaskBatches).toHaveBeenLastCalledWith(ID, { page: 1, pageSize: 20, status: 'FAILED', includeSplit: true })
  ...
  expect(api.listDownloadTaskBatches).toHaveBeenLastCalledWith(ID, { page: 2, pageSize: 50, status: 'FAILED', includeSplit: true })
  await flow.load(OTHER)
  expect(flow.batchStatus.value).toBe('')
  expect(flow.includeSplit.value).toBe(false)
  expect(api.listDownloadTaskBatches).toHaveBeenLastCalledWith(OTHER, { page: 1, pageSize: 20 })
})

it('coalesces changing batch filters and ignores old batches and errors', async () => {
  flow = useDownloadTask()
  await flow.load(ID)
  ...
  flow.changePage(2)
  flow.changeBatchFilters({ status: 'FAILED' })
  flow.changeBatchFilters({ status: 'SPLIT', includeSplit: true })
  expect(await flow.changeBatchFilters({ status: 'QUEUED' })).toBe(false)
  expect(await flow.changeBatchFilters({ includeSplit: 'false' })).toBe(false)
  expect(flow.batches.value).toBeNull()
  detail.resolve(task()); batches.reject(failure())
  await flushPromises()
  expect(api.listDownloadTaskBatches).toHaveBeenLastCalledWith(ID, { page: 1, pageSize: 20, status: 'SPLIT', includeSplit: true })
  expect(flow.batchesError.value).toBeNull()
  latest.resolve(task())
  await flushPromises()
  await flow.changeBatchFilters({ status: '', includeSplit: false })
  expect(api.listDownloadTaskBatches).toHaveBeenLastCalledWith(ID, { page: 1, pageSize: 20 })
})

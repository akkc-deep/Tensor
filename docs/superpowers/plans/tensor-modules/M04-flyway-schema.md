...
**Goal:** 通过固定 Flyway SQL 创建 49 张 Tushare 来源表、fixture 表、业务唯一键和查询索引，并验证实际 MySQL 8.4 schema 与 YAML 一致。
...
**Tech Stack:** MySQL 8.4 LTS、Flyway、SQL、Testcontainers MySQL 8.4。
...
- [ ] Apply V1–V2 to clean MySQL 8.4 and verify total source-table count 24.
...
- [ ] Apply V1–V3 to clean MySQL 8.4; expect 30 source tables.
...
- [ ] Apply V1–V4 to clean MySQL 8.4; expect 39 source tables and no index-length error.
...
- [ ] Apply V1–V5 to clean MySQL 8.4; expect exactly 49 `tushare_pro__*` tables.
...
- [ ] Write a Testcontainers MySQL 8.4 test that runs Flyway, loads 49 M03 YAML definitions and compares names, order, JDBC types, nullability, primary/unique keys and query indexes against `information_schema`.
...
Run `mvn -f data-plane/pom.xml -pl tensor-app -am -Dtest=FlywaySchemaContractIT test`. M04 is complete only when a clean MySQL 8.4 instance reaches 49 production tables, every YAML/schema comparison passes and production resources exclude fixture DDL.

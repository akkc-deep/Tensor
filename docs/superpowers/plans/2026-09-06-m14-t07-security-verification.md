...
- [x] Implement the self-test cases first using the same callable scanner, HTTP/header verdict, report-verdict and finalization functions used by the actual run. Observe rejection for a deliberately unsafe initial implementation before enabling formal execution.
...
- [x] Cover raw/encoded secret injection in source, nested ZIP, HTTP, logs and artifacts; unreadable/missing files; corrupt ZIP; malicious HTML DOM; permissive HTTP/header/Actuator responses; high/critical or failed/missing dependency reports; empty test reports; and failed cleanup. Clean controls pass, each injected defect fails through the real decision function.
- [x] Implement small named helpers inside the one script: input identity/tool validation; sanitized subprocess execution; recursive JAR scanning; test/audit report validation; Docker/defaults/SQL lifecycle; loopback stub and production JVM lifecycle; S01～S08 browser/HTTP probes; final scan and cleanup. Avoid a generic framework or arbitrary runtime selection flags.
- [x] Make preflight inspect hash/profile/resource identities, effective Node/Java versions, actual MySQL source-host permissions, and port ownership. Use the exact public-source snapshot and commands in the Spec for Maven/npm scans. Capture all scanner output privately and project only a reviewed summary.
- [x] Supplement the specified Dependency-Check aggregate command with `-DscanDirectory="$M14_SECURITY_LIBRARY_DIR"`, where the script privately copies the exact frozen nested JAR bytes. This covers Boot's automatically packaged `spring-boot-jarmode-tools-3.5.16.jar`, which is absent from reactor dependency resolution. The official aggregate CLI supports `scanDirectory`; retain all specified dependency scopes, updating, CVSS and error gates. Require valid report coverage for all packaged third-party libraries, not merely that extraction succeeded.
- [x] Retain the original child exit code for every gate, complete independent gates when safe, and always run finalization. Invalid security responses remain failures even if the product ignores parameters. Never fix production or change an assertion to accept an observed defect.
- [x] Expose detailed safe per-probe outcomes in the final report (including each S05/S06/S07 request) so real failures are reproducible without raw response dumps. Report expected probes not reached as not-run.
- [x] Verify:
...
- [x] Select installed Node 24 and Java 21, verify the frozen JAR, current Git identity, Docker/MySQL/browser availability, and clean protected inputs. Preserve prior untracked artifacts. No real credentials are needed.
- [x] Execute from repository root with only the approved JAR input and safe tool paths:
...
- [x] Record actual safe results. Scanner/network errors remain failures with exact stages; no suppressed errors, no fabricated report, no dependency upgrade. Correct harness defects with focused same-function counterexamples, re-review changes, then rerun only the affected independent gate where evidence integrity allows; full exit-0 acceptance still needs one coherent complete run.
- [x] Verify all owned resources and secrets are cleaned and the final evidence scan succeeded. Commit implementation evidence separately from board/control records as `test(security): record release control verification results`.
- [x] If all outcome-level criteria pass, mark M14-T07 COMPLETED before inspecting the predefined successor, then follow the required successor-design/handoff gates. Otherwise record the actual blocker and resolution in a pause handoff, then transition IN_PROGRESS→BLOCKED. Do not start M14-T08 or mark release ready while prerequisites remain unresolved.
...
实施与本轮执行、证据记录及失败交接步骤已完成；这些勾选不表示安全验收通过。正式轮源提交741376b，退出1，18通过/6失败/0未执行；扫描后的原始报告单独提交67b1be6。六项阻塞登记ISSUE-010～015，权威任务M14-T07按暂停交接转为BLOCKED，M14-T08未启动。

终审唯一检查器问题F1已在0faf9dd修正并通过一次专项复审；25项清理反例、309项离线自检及语法检查通过。对原始运行的删除后响应仅做新判定函数的语义验证，未重跑全流程、重做凭证扫描或改写报告身份。未来完成验收仍需完整运行退出0。

后续用户处置（2026-09-07）：用户明确要求“先标记完成吧，后续我单独处理这些问题”。权威看板按本次例外将M14-T07直接从BLOCKED标为COMPLETED，表示任务收尾；ISSUE-010～015全部保留未解决，交由用户后续单独处理。原安全验收结果及证据不变，本次未重跑验证，未准备或启动M14-T08。

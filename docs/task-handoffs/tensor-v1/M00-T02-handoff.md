...
- `docs/task-handoffs/tensor-v1/M00-T02-handoff.md`：以本次 `pause` 快照替换原 `next-task` 快照，记录设计阻塞与恢复入口。
- `docs/task-handoffs/tensor-v1/tensor-v1-task-board.md`：本交接写入后记录 `READY -> BLOCKED`、阻塞证据和最新交接路径。
...
1. `docs/task-handoffs/tensor-v1/tensor-v1-task-board.md` 的 `M00-T02` 行与任务详情。
2. `docs/superpowers/plans/tensor-modules/M00-contracts.md` 的 M00-T02 任务卡。
3. `docs/task-handoffs/tensor-v1/M00-T02-handoff.md` 全文。

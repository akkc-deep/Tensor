...
- `docs/task-handoffs/tensor-v1/tensor-v1-task-board.md`：记录用户显式启动证据并将 M00-T01 从 `READY` 转为 `IN_PROGRESS`；本交接写入后将记录 `IN_PROGRESS -> BLOCKED` 及交接路径。
...
1. `docs/task-handoffs/tensor-v1/tensor-v1-task-board.md` 的 `M00-T01` 行与任务详情。
2. `docs/task-designs/M00-T01-designs.md` 全文。
3. `docs/task-handoffs/tensor-v1/M00-T01-handoff.md` 全文。

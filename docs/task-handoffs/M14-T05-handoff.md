...
**最新用户安排：** 暂缓 M14-T05，将原 49 中尚未覆盖的 9 项交由 `docs/issues/problems/ISSUE-008-tushare-live-coverage-gap.md` 跟踪，先推进 M14-T06。本任务保持 BLOCKED，本轮无真实调用或状态转换。以下 M14-T09 入口及旧轮诊断文字仅为历史；M14-T09 已完成，不再恢复旧任务或运行已使用控制器。
...
原M14-T05的49接口目标保留，M14-T09的2000档工作已完成。此次不恢复M14-T05；依据用户新指示，当前工作入口为M14-T06的设计与性能验证。

## Start Here

1. `docs/issues/problems/ISSUE-008-tushare-live-coverage-gap.md`：9项缺口、历史依据和未来关闭条件。
2. 权威看板Order77/M14-T06及其精确设计引用（设计准备阶段尚未建立引用时，先消费该行指定任务卡）。
3. M14-T09完整设计和最终证据作为已完成输入；本任务旧真实证据仅作历史和技术输入。

首动作：按当前看板推进M14-T06；ISSUE-008后续恢复时先核对权限和完整验收合同，不复用旧启动器。

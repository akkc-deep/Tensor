...
| 78 | M14-T07 | Token、SQL、依赖、网络和运行安全验证 | `COMPLETED` | M14-T02, M14-T03, M14-T04, M14-T05 | docs/task-designs/M14-T07-design.md | docs/task-handoffs/M14-T07-handoff.md |
...
- **Completion evidence:** 2026-09-05：严格 TDD 先完整创建 `DownloadResult.spec.js`，聚焦命令仅因 `DownloadResult.vue` 缺失在收集阶段退出 1；提交 `8bb9959` 以固定消息精确新增 `DownloadAction.vue`、`DownloadResult.vue` 和 6 项真实组件测试，交付受控提交按钮、SUCCESS 三计数礼貌播报、EMPTY 精确成功文案、FAILURE 安全摘要/可用请求 ID/受 `canRetry` 唯一控制的原参数重试。逐项本地审查发现原测试未排除组件自行读取 `error.retryable`，提交 `c78d4f3` 以相反的内部 retryable/`canRetry` 值强化边界；错误实现突变产生精确 1 项失败，恢复受控实现后聚焦 6/6。最终新鲜验证在 Node.js 24.15.0 下聚焦 1 file / 6 tests、完整前端 12 files / 67 tests 全通过，Vite 8.2.2 构建转换 1599 modules 并退出 0，仅有既有 Element Plus chunk-size 提示；累计差异精确为设计三文件，初始实现提交范围与消息准确，安全扫描无命中且退出 1，受保护路径无差异，`git diff --check` 通过并且工作树干净。结果级检查确认所有公开 props/emits、锁定、三态、无障碍、纯文本和不泄漏边界逐项满足设计与任务卡，未混入排除范围，因此执行 `IN_PROGRESS -> COMPLETED`。
...
- **State evidence (start):** 2026-09-05：用户明确要求按照权威任务看板执行当前任务；已完整读取 `docs/task-designs/M11-T05-design.md`、`docs/task-handoffs/M11-T05-handoff.md`、M11-T05 任务卡、M11-T01～T04 直接依赖设计及当前组件/composable/API/页面/router 产物，并核对 PRD 4.1～5.7、7.2、9、10.5 和 12.1。任务身份、唯一流程、六段顺序、严格 TDD、精确四文件范围、验收和首个动作均可定位且无冲突；普通 `main` 检出由仓库级 `AGENTS.md` 明确授权原地工作，范围外 `.idea/misc.xml` 与 `data-plane/**/target/` 变化将保留并绕开。该请求构成本次 `READY -> IN_PROGRESS` 的明确启动证据；既有 `next-task` 交接路径保留为进入上下文。
- **Completion evidence:** 2026-09-05：严格 TDD 先只新增完整 8 项 `DownloadView.spec.js` 并调整 `App.spec.js`/`AppLayout.spec.js` 的元数据 API 隔离与新页面断言，在生产页面仍为占位时，三文件聚焦 12 项中 10 项仅因缺少自动加载、六段组件、状态和动作而失败，另 2 项既有路由行为通过；实现提交 `dda4f9d`（`feat(ui): complete data download page`）精确包含设计规定的一新增、三修改文件，交付唯一 `useDownloadFlow()`、mounted 来源加载、来源→接口→说明→参数→动作→结果顺序、元数据/下载 FAILURE 分流、校验—新鲜快照—提交、全控件锁定、最终三态、切换清理和真实键盘顺序。独立审查唯一 Important 指出元数据按钮直接绑定函数会转发 DOM event；Vue 3 编译器核验后由提交 `e84f21c` 改为显式无参 `retry()`，同一审查者范围化复审确认该发现已解决且无新 Critical/Important，结论 `Ready to merge: Yes`。Node.js 24.15.0 提交态最终新鲜验证为完整前端 13 files / 75 tests、页面/壳/router 聚焦 4 files / 15 tests 全通过；Vite 8.2.2 构建转换 1676 modules 并退出 0，只含项目所有者已批准的 Element Plus chunk-size 提示。禁止 HTML 注入、直连网络、错误展开、凭证、持久化、定时器、阶段、进度和具体来源/API 分支扫描无命中，依赖、配置、API、composable、子组件、工具、router、布局生产代码和样式无差异，`git diff --check` 通过，净范围精确为四个设计文件，`control-plane` 工作树无未提交变化；范围外 `.idea/misc.xml` 与后端 `target/` 输出原样保留。因此结果级目标、公开合同、无障碍、安全、测试和精确范围均满足，执行 `IN_PROGRESS -> COMPLETED`。
...
- **State evidence (start):** 2026-09-05：用户明确要求按照权威任务看板执行当前任务；已完整读取 `docs/task-designs/M12-T02-design.md` 与 `docs/task-handoffs/M12-T02-handoff.md`，并核对 M12 Global Constraints、Task M12-T02、Module Gate、PRD 6.5/7.4/AC-015、TRD 12.4/13.5、OpenAPI 数据集/分页合同、M10-T04 设计及当前格式化产物，确认任务身份、精确两文件范围、冻结公开接口、显示/安全边界、严格 TDD 顺序、验收与首个动作均可定位且无冲突。该请求作为本次 `READY -> IN_PROGRESS` 的明确启动证据，既有 `next-task` 交接路径保留为进入上下文。

- **Completion evidence:** 2026-09-05：严格 TDD 先只创建完整六项 `DatasetTable.spec.js`，聚焦命令在收集阶段仅因 `DatasetTable.vue` 不存在而非零；初始实现提交 `1f073ee` 以固定消息 `feat(ui): render complete dataset tables` 精确新增设计批准的两个文件。实现只消费 `columns/items/loading`，按输入原序追加三个来源列，复用 `formatCell`，以 140/240/180px 最小宽度、纯文本 tooltip、横向滚动和 loading/ARIA 覆盖 152 业务列。独立审查发现 Element Plus 原生 `fixed` 会把非首位 `ts_code` 移到表头最前；修复提交 `c75b4eb` 先以真实非首位 `ts_code` DOM 断言取得失败，再用仅作用于目标表头/表体单元格的 sticky 定位同时保持元数据顺序与左侧固定，并补齐空业务列及格式化入库时间 tooltip 合同。复审发现普通单元格与 sticky 单元格层叠级别相同；修复提交 `cd8f7ab` 先取得数值层级失败，再改为 Element Plus 同源的 `calc(var(--el-table-index) + 1)`。最终提交上 Node.js 24.15.0 聚焦测试 1 file / 6 tests、完整前端回归 17 files / 98 tests 全部通过，生产构建退出 0 且只有既有 chunk-size 提示；格式、完整暂存区、受保护路径和精确两文件提交范围检查通过，禁止 HTML 注入、网络、具体数据集分支、排序/选择及数值转换扫描无输出并按预期退出 1。最终独立复审无 Critical、Important 或 Minor，结论 `Ready to merge: Yes`；因此结果级目标、范围和全部验收满足，执行 `IN_PROGRESS -> COMPLETED`，既有交接路径保留为历史进入上下文。
...
- **Completion evidence:** 实施按严格 TDD 先只创建完整资源合同测试，批准的聚焦命令在上游无匹配测试继续后于 `tensor-app` 精确因 `target/generated-resources/static/index.html` 不存在而 RED；提交 `3296877`（`build: integrate frontend assets into app`）精确以一修改一新增文件固定 frontend-maven-plugin 1.15.4、Node v24.15.0、npm 11.12.1、`npm ci → 20 files / 120 tests → Vite 8.2.2 build → maven-resources-plugin 3.4.0 unfiltered copy` 顺序及复制后入口/哈希 JS/CSS 合同。任务级审查确认规格符合、质量 `Approved` 且无 Critical/Important/Minor；最终整体审查复现非 clean 构建旧/新 hash 并存时任取首文件的误失败，修复提交 `aa7f55d` 增加真实临时文件回归并只接受同时满足 hash 形状与入口引用的资源，范围化复审确认原 Important 已解决且无新 Critical/Important 或范围外发现。最终提交树的新鲜聚焦生命周期在 Node v24.15.0/npm 11.12.1 下通过前端 120/120、Vite build、四资源复制及 Java 2/2；沙箱内完整回归仅因未改动的 Mockito/Byte Buddy JVM agent 无法自附加失败，沙箱外原样命令随后通过模块 79/75/93/12/81，均为 0 failure、0 error、0 skipped。生成目录含 `index.html`、其引用的哈希 JS/CSS 与 favicon；lockfile、前端源码/配置和根 POM 无差异，`git diff --check` 与精确两文件范围通过，宽泛 Git 扫描唯一命中强制坐标 `com.github.eirslett`，边界化扫描确认无 Git 命令、`.git` 路径或 `GIT_*` 探测；范围外 `.idea/misc.xml` 与 Maven `target/` 均保留。因此结果级验收满足，执行 `IN_PROGRESS -> COMPLETED`。
...
- **Completion evidence:** 实施按严格 TDD 先只创建完整 `PackagedJarContractTest`，批准的 `clean ... test` 命令在前端 20 files / 120 tests 和 app 测试选择均成功后，仅因 `target/tensor-app-1.0-SNAPSHOT.jar` 在 package 前不存在而 RED。提交 `c9ae5b1`（`build: package Tensor as one executable jar`）精确以一修改一新增文件注册两个未过滤主资源根、配置 Spring Boot 3.5.16 原位 repackage，并用 Surefire exclusion/Failsafe 3.5.6 固定 post-package 内容合同；任务审查发现的 Tushare 模块所有权缺口由 `9c34e6e` 修复，最终整体审查发现的子目录漂移、重复 archive entry 与非法 UTF-8 缺口由 `0032094` 及三个定向负例修复，两次范围化复审均确认全部 findings addressed 且无新 Critical/Important/Minor。主控在最终提交树上以正常 JVM 权限新鲜运行 `mvn -f data-plane/pom.xml -pl tensor-app -am clean verify`，前端 120/120，plugin-api 79、core 75、Tushare 93、fixture 12、app Surefire 81 和 Failsafe 合同 4/4 均为 0 failure、0 error、0 skipped，Boot `repackage` 与 reactor 退出 0；沙箱内唯一失败仍是未改动的 Mockito/Byte Buddy 自附加限制。产物为唯一 `tensor-app-1.0-SNAPSHOT.jar`，manifest 指向 `JarLauncher`/`TensorApplication`，外层可见 index、哈希 JS/CSS、精确 V1–V5 和三个生产内层 JAR且禁止项扫描为空；Java 合同实际打开内层 JAR验证 49 个直接子级 Tushare YAML 只属于 Tushare 模块，并拒绝 fixture/V6/测试/报告/凭证、重复 entry 与非法 UTF-8。净实现范围仍精确为 POM 修改和合同测试新增，受保护路径无差异，`git diff --check`、Git 跟踪和禁止 Git 能力门禁通过，范围外 `.idea/misc.xml` 与生成 `target/` 未提交。因此满足设计和任务卡的结果级验收，执行 `IN_PROGRESS -> COMPLETED`。
...
- **Completion evidence:** 2026-09-05：实现提交 `5a2237d`（`feat(app): configure production web delivery`）精确包含一 YAML 修改、两个 Java 新增；先写完整测试，聚焦命令在前端 120/120 和资源复制通过后仅因缺少生产配置类型出现 testCompile RED。最小实现提供 GET/HEAD 无扩展名 UI internal forward、保留 API/Actuator/assets/文件型空 404、既有六个安全头和冻结缓存策略，以及默认关闭、单 origin、非 credential 的 API CORS；YAML 增加 graceful shutdown、每阶段 70s 与 dev origin，既有 120s 上游/130s 前端/60s 事务不变。Spring 6.2.19 实际会拆分逗号 origin 并去除末尾斜杠，独立探针揭示潜在多 origin/wildcard；据既定 fail-closed 合同对这两类错误值不注册 CORS，并以 `34b7260` 同步设计/计划。完整回归发现测试辅助配置被非 Web Boot 测试扫描，已仅在新测试内改为显式注册、方法局部 probe，生产组件扫描探针确认新 SPA 与三个真实 API 控制器可发现且测试 helper 排除。五项规定 mutation 均触发预期失败，恢复后聚焦通过；任务审查要求的两个 malformed wildcard 用例及 POST UI 不 forward 用例由 `1b599c2` 补齐，最终聚焦 28/28，范围复审全部 addressed。主控于 13:20 在最终源码树上以正常 JVM 权限新鲜执行 `mvn -f data-plane/pom.xml -pl tensor-app -am clean verify`，退出 0、BUILD SUCCESS：前端 120/120，plugin-api 79、core 75、Tushare 93、fixture 12、app 109 共 368 项 Surefire，Failsafe JAR 合同 4/4，全部零失败、零错误、零跳过，Boot repackage 成功。沙箱原命令的 Byte Buddy self-attach 限制已确认，未改 JVM/Mockito、未删测或 skip；保留既有编码、Vite chunk、动态 agent 及预期负例诊断。最终整体审查 `Ready to merge: Yes`，无 Critical/Important/Minor。提交范围、受保护路径、格式、Git 跟踪、禁止项和 JAR 检查通过，`.idea/misc.xml` 与生成 `target/` 未提交。结果级验收满足，执行 `IN_PROGRESS -> COMPLETED`；代理响应超时 `>=130s` 与按 Spring 每阶段停机语义安排外部终止窗口作为 M13-T04 强制输入。
...
- **State evidence (completion):** 2026-09-05：实现 `23addbe`（`test(e2e): verify fixture user flow`），精确新增 `control-plane/e2e/fixture-flow.spec.js` 与 `docs/verification/M14-T01-fixture-flow.md` 两个100644文件。Java21.0.11/MySQL8.4.6/Node24.15.0/npm11.12.1/Playwright1.62.1/Chromium151.0.7922.34真实环境，原生产者acceptance clean verify通过前端120/Surefire368/Failsafe7。最终原生产JAR在独立空库健康与两页直接访问/刷新通过后，仅因公开Fixture选项缺席失败（后两项串行未执行）；最终验收JAR在另一全新空库以规定Playwright命令恰3 passed/15.7s，0failed/skip/retry。页面SUCCESS与响应1/1/0，证券代码查询唯一七列、日期/精确数值/null/来源/上海显示时间正确；EMPTY0/0/0且原行含ingested_at完全不变；同包同库false重启后两页fixture缺席，Tushare完整摘要相等。恰两次页面点击POST，无mock/直接业务API替代/SQL种数/外部浏览器请求/非预期JS或业务HTTP错误。两个最终JVM正常SIGTERM、自行close、8080关闭，日志确认Web与连接池清理；只读复核V1～V6六条成功迁移/50业务表/fixture一行。最终前端120/120、语法、三用例发现、范围、格式、Git精确两新增门禁通过；五PNG已人工脱敏查看，本地PNG/JSON实际路径与SHA-256、行值/摘要及失败定位均在证据文档，已扫描无本次随机凭证。任务独立审查PASS/APPROVE，最终整体审查Ready to complete: Yes，无Critical/Important/Minor遗留；仅scratch报告状态过期已更新。无生产/config/runbook修改，不宣称后继或真实Tushare已验收。按结果级验收执行 `IN_PROGRESS -> COMPLETED`，保留原交接作历史入口；后继设计准备属于此完成记录之后的独立操作。
...
- **State evidence (D-02 restart):** 批准/就绪与BLOCKED→READY已独立提交cf64556。消费完整且hash匹配的批准设计与同路径pause交接，按用户持续执行当前任务及明确D-02“同意”的授权，观测READY后单独执行READY→IN_PROGRESS。正式入口为新控制yx5keenc，完整40/48/80及fixture2/3；失败停止、原参数/九排除与冻结包不变。本转换表示开始复验，不预先声明全40或top10_holders页面末查通过。

- **State evidence (D-02 outcome / D-03 block):** yx5keenc实际证据先独立提交6ae877f，整篇SHA6f91d9cbe0f4469a1278ede8de65b56f5841b292141202b88e97f2238948f695与真实秘密扫描标记一致。150秒、npx/最终exit1，33通过/1失败/6未运行。D-02的top10_holders SUCCESS320/320/0、页面末查与独立DB320，完整通过；dividend38继续通过。新失败top10_floatholders SUCCESS280/280/0、独立DB280，但当前EMPTY断言失败，仅初查0、末查未执行。真实POST42/records67、fixture2/3，114请求各恰一个完成事件；初始7迁移50全空，已完成33项页面/独立DB匹配、九排除表0，全40匹配false。扫描/worker/JVM/自有容器卷和私密DB材料清理通过，8080空闲。D-03具体修订待确认，当前spec仍20499e...且仅双覆盖，不自动改预期/复跑。完整读取更新设计，先写同路径pause交接，再观测IN_PROGRESS并执行IN_PROGRESS→BLOCKED；原49未完成，不准备后继。

- **Outcome review:** D-02完整修改与本轮结果/状态/交接独立复核PASS，无剩余发现；证据SHA、33/1/6、页面/独立DB匹配和D-03待确认边界一致。任务仍BLOCKED；实际证据、冻结spec/JAR/manifest与旧证据均保持扫描后hash，已清理运行产物仅保留4个白名单文件。
...
- **State evidence (owner-directed completion, 2026-09-07):** 用户已获知六项问题及功能影响后，明确要求“先标记完成吧，后续我单独处理这些问题”。已完整读取既有设计及暂停交接，观测BLOCKED；按本次明确例外直接记录BLOCKED→COMPLETED，不补造解除阻塞、READY/IN_PROGRESS或验收通过证据。此完成仅表示按用户决定收尾，原设计安全验收仍未满足；ISSUE-010～015保持未解决并交由用户后续单独处理，正式报告18pass/6fail/0not-run、退出1及SHA保持不变。保留原交接为历史快照，本次未重跑验证或修复问题，未准备或启动M14-T08。

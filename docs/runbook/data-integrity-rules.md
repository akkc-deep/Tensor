...
版本：1.0；核对日期：2026-09-15；适用范围：当前 40 个 Tushare 接口。

第一次阅读可以先看[逐项详解与例子](data-integrity-explained.md)，其中解释了每个规则编号及 40 个接口的判断方法。

本文是一套检查与验收规范。表中“当前行为”来自仓库代码；检查报告、巡检频率和源端全量对账是建议执行的规则，不表示应用已经提供相应功能，也不代表实际数据库已通过检查。

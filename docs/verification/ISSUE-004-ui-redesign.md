...
最终审查接受一项非阻塞Minor：`control-plane/e2e/ui-redesign.fixtures.js`的限定单行YAML解析器尚未严格验证整行消费，未来未知属性可能被吸收到相邻字符串值。现有49项生产输入已核对只使用支持的键，不影响本轮数据或正式前端。后续扩展该helper或列格式时，应完整校验限定语法并拒绝未知/重复键，补充对应反例；本次按非阻塞测试输入维护项延期，不扩大为通用YAML解析器。

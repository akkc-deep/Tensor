...
按[用户明确决定](../issues/proposals/ISSUE-026-range-scope.md)，本次排除balancesheet/cashflow/repurchase/fina_indicator的RANGE批量下载；当前30个RANGE接口、251项TASK纳入，原27项问题保留且不计PASS。此限定适用于下文及母任务的34/278全量关闭要求，其余真实证据、SINGLE、兼容及最终门禁继续适用；032不启动，本任务当前状态不变。

...
| 源/接口/数据集 | combobox 名称 exact `数据源`/`数据接口`/`数据集`；focus→Enter→可见 option.click。源 `Fixture` 或 `Tushare Pro`；接口/数据集 option `/Fixture 日线.*fixture_daily/`、`/日线行情.*\(daily\)/`；范围用 `/IPO 新股发行信息.*new_share/`。不使用 Element Plus CSS、force 或内部 Vue 状态。 |

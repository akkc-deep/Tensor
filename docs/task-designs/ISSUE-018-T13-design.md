...
按[用户明确决定](../issues/proposals/ISSUE-026-range-scope.md)，四接口balancesheet/cashflow/repurchase/fina_indicator本次记EXCLUDED，30个RANGE接口纳入、6个原SINGLE_ONLY保持。原34项目录矩阵和全部历史不改，251项通过与27项排除分开记录；本节限定替代下文全34必须通过的关闭要求，其余验收不变。032不启动，本任务当前状态不变。

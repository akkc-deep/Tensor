...
2026-09-14限定修订：用户在ISSUE-025明确“可以接受不完整”，仅对[决定中列出的十一接口](../issues/proposals/ISSUE-025-extraction-contracts.md#决策记录)采用RESPONSE_ONLY。它替代本设计§3.4–3.5、§5真实接口验证、下列第5/8项及§7对这些接口必须证明完整提取的要求；原文保留作差异依据。独立规则/评估、单次响应处理、校验/适配/入库、空与失败含义、持久化快照及页面说明按[T13限定合同](ISSUE-018-T13-design.md#issue-025-响应采集限定采用2026-09-14)执行。UNKNOWN仍不能直接成功，RESPONSE_ONLY只表示本次返回记录已采集，不能显示完整下载；其他接口的阈值/日历、事务/失败、真实TASK/SQL及全部门禁不变。生产实现由ISSUE-026完成，本决定不关闭母issue。

...
处理中，未解决（2026-09-11）。已完成当前 40 个接口的官网文档调研及代码现状核对，并整理[方案草稿](../proposals/ISSUE-018-date-range-batch-downloads.md)和[详细设计](../../task-designs/ISSUE-018-design.md)。当前确认方向为后台执行、按批入库、批次状态持久化、手动重试及多数据源复用；T01–T05 已完成插件合同 / 参数绑定、持久化仓储、本地接收与查询、证券数据与批次状态原子提交、Tushare 请求上下文与共享节流，并记录验证证据；来源区间策略、后台执行、HTTP / 前端及真实完整性验收仍由后续任务实施。下文保留调研依据。

已依据详细设计拆为 13 项任务，见[ISSUE-018 任务看板](../../task-handoffs/ISSUE-018/ISSUE-018-task-board.md)。当前 T01–T05 为 COMPLETED，T06 的设计和交接已就绪、状态 READY；准确状态以看板为准。真实接口完整性验收与基础设施验收分别跟踪。

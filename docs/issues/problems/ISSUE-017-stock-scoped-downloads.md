...
四接口本次RANGE批量下载按[用户决定](../proposals/ISSUE-026-range-scope.md)排除，不再构成本次RANGE关闭前置；全部40接口SINGLE与本issue股票归属要求保持。以下旧阶段状态保留，最终一致性和相关验收仍待032，本次不启动032或关闭本issue。

## 当前状态

已解决（2026-09-15，本次批准范围）。40接口SINGLE、34股票/6非股票规则与完整74任务/148查询及SQL成立；30个RANGE接口/251项通过，四接口27项明确排除并保留问题。T14/T13及032已完成，六门禁和独立终审通过，关闭条件逐条见[最终验收](../../verification/ISSUE-032-range-final-closure.md)。没有提交或发布。以下旧阶段结论保留为历史。

## 历史阶段状态

处理中（更新于2026-09-13）。[26 项必填股票参数方案](../proposals/ISSUE-017-required-stock-parameters.md)已实施：34 项必须指定股票，6 项保留原下载方式；参数绑定、响应归属、前后端回归及生产构建已通过，见[实施与验证记录](../../verification/ISSUE-017-stock-scoped-downloads.md)。`fina_mainbz` 的 SINGLE 参数已由 [ISSUE-018-T06](../../task-handoffs/ISSUE-018/ISSUE-018-task-board.md#issue-018-t06) 纠正为仅 `ts_code`，自动化验证通过。T14已在独立新库完成40API/74个真实SINGLE任务/148次查询，全部PASS且exit0/cleanupPASS，SQL两股票归属及历史保留验收成立；T13旧失败轮次与15个任务保留。ISSUE-020新SOURCE已观察默认P/D/I三类，且RANGE也返回110/150行；两股票报告期整段/边界已补齐，用户已同意方案A，默认保留上游分类、SINGLE单次快照、RANGE100工程阈值；任务/SQL后续验收归ISSUE-026。详见[T14后续证据](../../verification/ISSUE-017-stock-scoped-downloads.md#issue-018-t14-完整两股票验收2026-09-13)，母issue不关闭。
...
- [x] 对股票下载以两只不同股票分别验证，返回及本次入库的行均属于对应股票；混入其他股票的模拟响应被拒绝且整批不入库。合法空结果不触发扩大范围请求。
- [x] `top_list` 必填交易日、`fina_mainbz` 报告期参数、`index_member_all` 股票/行业代码区别等差异均有对应验证；长区间结果不得无依据宣称完整。
- [x] 为 34 项股票下载记录带股票条件的真实验收及结果范围，并为 6 项原方式下载记录对应回归证据；权限不足、停止更新、没有合适历史样本等情况如实记录，不能用官网支持或旧验收替代真实通过。
- [x] 元数据、接口合同、当前下载样例、前后端回归及相关契约校验与新范围一致，更新本 issue 状态并链接验收记录。
